...
- [x] `.github/workflows/ci-cd.yml` reescrito: typecheck/lint/test/build reales contra secrets de GitHub Actions (a configurar por el usuario) + job separado de validación de builds Docker. Los 3 primeros runs reales fallaban en Typecheck (`packages/database/generated/` nunca se regeneraba en un checkout limpio); corregido con un `postinstall` real (ver `DECISIONS.md`) — confirmado localmente reproduciendo el mismo fallo con una reinstalación desde cero.
