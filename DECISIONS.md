...
## 2026-07-18 — Bug real de CI: `packages/database/generated/` nunca se regeneraba en un checkout limpio

**Decisión**: `packages/database/package.json` agrega `"postinstall": "prisma generate"`, así que el cliente de Prisma se regenera automáticamente en cualquier `pnpm install` (CI, clone nuevo, Vercel, Railway), no solo cuando alguien lo corre a mano. Los dos `Dockerfile` (`backend`, `frontend`) ahora copian `packages/database/prisma` (el schema) junto a los `package.json` **antes** del `RUN pnpm install --frozen-lockfile`, para que el postinstall tenga lo que necesita en ese punto de la imagen — antes solo copiaban `package.json` en esa etapa y el resto del código fuente después. El paso explícito `RUN pnpm --filter @network-learning-game/database run db:generate` que tenía el `Dockerfile` de backend se eliminó por redundante.

**Motivo**: bug real, no hipotético — los 3 runs de GitHub Actions hasta ahora fallaban en el step de Typecheck (confirmado vía la API pública de GitHub Actions, `gh` CLI no está instalado en esta máquina). `packages/database/generated/` está en `.gitignore` (el cliente de Prisma 7 se genera como fuente TypeScript, no se commitea) y nada en `ci-cd.yml` lo regeneraba antes de `pnpm typecheck` — reproducido localmente moviendo `generated/` fuera y corriendo `pnpm typecheck`, mismo error `TS2307: Cannot find module '../generated/prisma/client.js'`. Los Dockerfiles nunca sufrieron esto porque ya tenían su propio paso explícito de `db:generate`, lo que enmascaró el problema durante toda la Fase 6 (Docker "funcionaba", CI no, y nadie lo había cruzado hasta ahora). Verificado con una reinstalación real desde cero (`rm -rf node_modules` + `pnpm install --frozen-lockfile`, exactamente lo que hace CI) mostrando `packages/database postinstall$ prisma generate` corriendo solo.

**Efecto secundario**: `pnpm --filter backend deploy --prod /deploy` (usado en el `Dockerfile` de backend) empezó a fallar con `ERR_PNPM_IGNORED_BUILDS` al intentar re-aprobar el postinstall de `@network-learning-game/database` dentro del store aislado de `/deploy` — pnpm 11 bloquea scripts de build por defecto ahí. Como para ese punto del build `packages/database` ya está compilado (con su cliente de Prisma ya generado en un paso anterior), el deploy no necesita volver a correr ningún script — se agregó `--ignore-scripts` al comando `pnpm deploy`.

## 2026-07-18 — Bug real de plataforma: Prisma 7.8.0 genera imports `.ts` distintos en Linux vs Windows

**Decisión**: `packages/database/tsconfig.json` agrega `"rewriteRelativeImportExtensions": true`.

**Motivo**: bug real, reproducido y aislado con un experimento controlado, no asumido. La imagen Docker del backend arrancaba pero `GET /courses` devolvía 500 con `Cannot find module '.../internal/class.ts'` — el `.js` compilado por `tsc` contenía un import con extensión `.ts` literal, inválido en runtime ESM. Se aisló la causa comparando el mismo comando (`prisma generate`, cargando el mismo `prisma.config.ts`, mismo `schema.prisma`) corrido en tres contextos: en el host Windows (`.js` correcto), vía `npx prisma` suelto dentro de un contenedor Linux (`.js` correcto, pero sin cargar `prisma.config.ts` por falta de `dotenv`), y vía `pnpm --filter database run db:generate` dentro del build real de Docker (Linux, con `prisma.config.ts` cargado con éxito) — únicamente este último produjo `from "./internal/class.ts"` en el *código fuente* generado, antes de que `tsc` lo tocara. Es decir: el generador de Prisma 7.8.0 emite un source distinto según la plataforma en la que corre (o alguna condición que correlaciona con ella), no un bug de `tsc` ni de caché de Docker (se descartó cachear con un rebuild `--no-cache` completo y con `docker builder prune`). En vez de perseguir la causa exacta dentro del generador de Prisma (código de terceros, cerrado), se optó por hacer que nuestro propio compilador tolere cualquiera de las dos formas: `rewriteRelativeImportExtensions` hace que `tsc` reescriba `.ts`→`.js` en imports relativos del output sin importar cuál use el source, sea cual sea la plataforma que generó el cliente.

## 2026-07-18 — Bug real: `DATABASE_URL` apuntaba a la conexión directa de Supabase, no a la pooled

**Decisión**: `DATABASE_URL` en `.env` ahora usa la conexión pooled real de Supabase (`aws-0-<region>.pooler.supabase.com:6543`, usuario `postgres.<project-ref>`), no la misma URL que `DIRECT_URL` (`db.<project-ref>.supabase.co:5432`).

**Motivo**: bug de configuración real, ya documentado como intención en `CONTRIBUTING.md`/`.env.example` (`DATABASE_URL: conexión pooled... DIRECT_URL: conexión directa`) pero nunca aplicado de verdad — ambas variables tenían el mismo valor desde Fase 2. Invisible en este host (tiene ruta IPv6 de salida funcionando hacia Supabase, así que `pnpm dev`/tests nunca lo notaron), pero rompía por completo el contenedor Docker del backend corriendo contra la red NAT de Docker Desktop (sin salida IPv6): `ENETUNREACH` al conectar a la dirección IPv6 que resuelve el host de conexión directa. La conexión pooled de Supabase soporta IPv4 explícitamente por esta razón — es el patrón recomendado por Supabase para runtime de aplicación, reservando la conexión directa solo para `prisma migrate` (que si corre desde este host con IPv6 funcional). Relevante más allá de Docker local: Railway (destino de deploy real, ver `ROADMAP.md`) tampoco garantiza salida IPv6, así que este fix era necesario antes de intentar ese deploy, no solo para uso local de `docker compose`.

## 2026-07-19 — Bug real de CI: Turborepo no pasaba los secrets de GitHub Actions a los tasks

**Decisión**: `turbo.json` agrega `globalPassThroughEnv` listando `DATABASE_URL`, `DIRECT_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `NODE_ENV`, `API_PORT`, `NEXT_PUBLIC_API_URL`.

**Motivo**: bug real confirmado leyendo el log crudo de un run fallido de GitHub Actions (`gh run view --log-failed`, instalado `gh` CLI específicamente para poder diagnosticar esto en vez de asumir — ver más abajo). Con los 5 secrets ya configurados en el repo y el `env:` a nivel de job en `ci-cd.yml` correctamente referenciándolos, `pnpm test` seguía fallando con `DATABASE_URL no está definido` y `SUPABASE_SERVICE_ROLE_KEY es requerido`, ambos lanzados desde dentro del proceso de `vitest` que corre `backend#test`. La causa: Turborepo 2.x usa `envMode: "strict"` por defecto, que solo deja pasar a los procesos de cada task las variables de entorno declaradas explícitamente (`globalEnv`/`env`/`globalPassThroughEnv`) — cualquier otra variable del entorno del shell, aunque esté seteada, se filtra antes de spawnear el proceso hijo. Localmente esto nunca se notó porque `apps/backend/test/setup.ts` carga `.env` directo del disco vía `dotenv`, sin depender de la herencia de `process.env` a través de Turborepo — un mecanismo completamente distinto al de CI, que inyecta los secrets solo como variables de entorno (no hay `.env` en el checkout, está en `.gitignore`). No se pudo reproducir el fallo localmente (Windows) simulando las mismas condiciones (sin archivo `.env`, variables seteadas a mano en el shell antes de `pnpm test`) — el filtrado estricto de Turborepo parece tener comportamiento distinto entre plataformas; se optó por aplicar el fix documentado oficialmente para este escenario exacto (secrets de CI que deben llegar a los tasks sin formar parte del hash de caché) y confirmarlo contra el run real de GitHub Actions en vez de insistir en reproducirlo localmente.

**Nota operativa**: esta máquina no tenía `gh` CLI instalado — se instaló vía `winget install GitHub.cli` y se autenticó con `gh auth login --web` (device code, autorizado por el usuario en el navegador) específicamente para poder leer logs reales de Actions en vez de adivinar causas de fallo a partir de reintentos ciegos.

## 2026-07-19 — Deploy real a Railway (backend)

**Decisión**: `railway.toml` en la raíz del repo declara `build.dockerfilePath = "apps/backend/Dockerfile"` (build context = raíz del repo, igual que `docker build -f apps/backend/Dockerfile .` local/CI) y `deploy.healthcheckPath = "/health"`. El servicio de Railway (`network-learning-game`, mismo nombre que el proyecto) requiere una variable `PORT=3001` — además de las ya documentadas (`NODE_ENV`, `DATABASE_URL`, `NEXT_PUBLIC_SUPABASE_URL`) — aunque `apps/backend/src/main.ts` nunca lee `process.env.PORT` (lee `API_PORT`, con default 3001).

**Motivo**: bug real observado desplegando de verdad, no anticipado. El primer deploy (sin `PORT` seteada) construyó la imagen correctamente y el contenedor arrancó sin errores — los logs mostraban `Nest application successfully started` y `escuchando en http://localhost:3001` — pero Railway marcó el deployment como `FAILED` y el dominio público devolvía `404 Application not found` de su edge proxy. La causa es que el healthcheck/detección de puerto de Railway para deploys basados en Dockerfile depende de la variable `PORT`, no de `EXPOSE` del Dockerfile ni de que la app realmente escuche en ese puerto — sin `PORT`, Railway no sabe a qué puerto dirigir el healthcheck aunque el target port del dominio se haya configurado explícitamente (`railway domain --port 3001`). Seteando `PORT=3001` (idéntico al valor real que ya usa la app vía `API_PORT`) el siguiente deploy pasó el healthcheck y quedó accesible públicamente — verificado con `curl` real contra `/health` (200) y `/courses` (datos reales del seed) en la URL pública (`https://network-learning-game-production.up.railway.app`).

**Nota operativa**: se instalaron los CLIs de Railway (`npm install -g --allow-scripts=@railway/cli @railway/cli`) y ya estaba disponible el de Vercel; ambos ya autenticados por el usuario (`railway whoami` / `vercel whoami` confirmados antes de desplegar). `railway setup agent` instaló un skill y MCP server para uso de agentes en sesiones futuras (requiere reiniciar la herramienta para cargar, no disponible en esta sesión) — quedó configurado pero esta sesión siguió usando el CLI directo.

## 2026-07-19 — Bug real: `pnpm-lock.yaml` era un stream YAML de dos documentos; rompía el build de Vercel

**Decisión**: se removió el bloque `devEngines` de `package.json` (raíz). Se regeneró `pnpm-lock.yaml` como un único documento YAML válido.

**Motivo**: bug real encontrado desplegando el frontend a Vercel, no anticipado — el build fallaba con `WARN Ignoring broken lockfile... expected a single document in the stream, but found more`, seguido de una cascada de errores `ERR_INVALID_THIS` al re-resolver dependencias sin lockfile (el pnpm del entorno de build de Vercel, más viejo, ignoraba el lockfile "roto" y intentaba resolver todo desde cero, chocando además con un bug de compatibilidad de ese pnpm más viejo contra el registry). La causa raíz: `pnpm-lock.yaml` tenía **dos documentos YAML concatenados** (dos líneas `---`, línea 1 y línea 199) — el primero era un lockfile separado de solo 197 líneas para `packageManagerDependencies` (`@pnpm/exe`, el binario de pnpm 11.13.1 auto-gestionado como dependencia, activado por `devEngines.packageManager.onFail: "download"` en `package.json`), el segundo (a partir de la línea 200) el lockfile real y completo del monorepo. Es un feature legítimo de pnpm 11 (fijar el binario exacto de pnpm como dependencia reproducible), pero produce un lockfile multi-documento que el pnpm del build de Vercel no soporta.

Se intentó primero desactivarlo vía `managePackageManagerVersions: false` en `pnpm-workspace.yaml` — **no funcionó**: verificado corriendo `pnpm install --frozen-lockfile` repetidas veces con esa opción puesta, el segundo documento volvía a aparecer en cada corrida pese a que `--frozen-lockfile` no debería reescribir nada. Se descartó esa opción (removida del repo, no dejarla como si funcionara) y se atacó la causa real: quitar `devEngines` de `package.json`, el campo que efectivamente dispara el auto-gestionado del binario. Confirmado con el mismo test repetido (`pnpm install --frozen-lockfile` varias veces seguidas) que el lockfile ya no vuelve a corromperse. No hacía falta para este proyecto de todas formas — Vercel/CI/Docker ya resuelven la versión de pnpm por su cuenta vía el campo `packageManager` + corepack.

**Nota**: este bug estaba presente en el lockfile commiteado desde antes de esta sesión de deploy — nunca se manifestó en `pnpm install --frozen-lockfile` local, CI (Ubuntu, pnpm vía `pnpm/action-setup@v4`) ni Docker (`corepack enable` + mismo pnpm pineado) porque todos usan una versión de pnpm lo bastante nueva como para leer el lockfile multi-documento sin quejarse — solo el toolchain de build de Vercel (que resuelve pnpm por su cuenta, versión no controlada por nosotros) lo rechazó. Otro recordatorio de que "funciona en CI/Docker" no prueba "funciona en cualquier plataforma de deploy" — cada una resuelve su propio toolchain de forma independiente.

## 2026-07-19 — Deploy real a Vercel (frontend): Root Directory + `.vercelignore` para el monorepo

**Decisión**: el proyecto de Vercel (`network-learning-game`) tiene `rootDirectory: "apps/frontend"` (seteado vía la API de Vercel, `vercel project update` no expone ese campo). Se agregó `.vercelignore` en la raíz del repo, espejando `.gitignore`, con `.git/` explícito además.

**Motivo**: dos bugs reales encontrados desplegando de verdad:
1. Vinculando el proyecto desde `apps/frontend` (`vercel link` corrido ahí), el CLI subía *solo* esa carpeta como fuente — sin `pnpm-lock.yaml`/`pnpm-workspace.yaml` de la raíz ni los packages hermanos, Vercel no detectaba pnpm y corría `npm install`, que fallaba porque `workspace:*` no es una especificación válida para npm. Corrido en cambio desde la raíz del monorepo (con `rootDirectory` ya seteado a `apps/frontend` vía API), Vercel detectó Turborepo/pnpm correctamente y corrió `pnpm install` desde la raíz real.
2. Sin `.vercelignore`, ese mismo deploy desde la raíz subió 319MB (incluyendo binarios nativos de `node_modules/.pnpm/@next+swc-win32-x64-msvc@.../*.node` de >100MB y caché de `.turbo/cache/*.tar.zst`) y falló por el límite de 100MB por archivo de Vercel — pese a que `.gitignore` ya excluye `node_modules/`/`.turbo/`, el CLI de Vercel no lo respetó igual desde la raíz del monorepo en este caso. `.vercelignore` explícito lo resolvió.

**Nota operativa**: el flag `-F`/`--field` del comando `vercel api` (usado para el PATCH de `rootDirectory`) requiere `MSYS_NO_PATHCONV=1` en Git Bash — sin eso, MSYS reescribe el path del endpoint (`/v9/projects/...`) como una ruta de Windows y la llamada falla con "Invalid arguments" (mismo problema de path-mangling que Docker con `-w /repo`, ver entrada de Fase 1/2).

## 2026-07-19 — Bug crítico real de seguridad: RLS deshabilitado en las 19 tablas de `public` — datos de producción escribibles con la anon key

**Decisión**: se agregó la migración `packages/database/prisma/migrations/20260719033822_enable_row_level_security/migration.sql`, que corre `ALTER TABLE ... ENABLE ROW LEVEL SECURITY` (sin `FORCE`, sin policies) sobre las 19 tablas de la aplicación más `_prisma_migrations`.

**Motivo**: Supabase Advisors (`SECURITY`/`ERROR`) reportó `rls_disabled_in_public` en todas las tablas — y esto **no era un hallazgo de linter, era explotable de verdad**: verificado con un `PATCH` real contra `https://<ref>.supabase.co/rest/v1/courses` usando únicamente la `anon key` (pública, embebida en el bundle del cliente, extraíble por cualquiera que inspeccione el sitio) — el título del curso sembrado se sobrescribió en la base de producción sin ningún token de sesión ni autorización, confirmando que cualquier visitante podía leer y escribir las 19 tablas (incluyendo `users`: email, rol, XP) saltándose por completo `apps/backend` (guards, `RolesGuard`, toda la lógica de autorización). Se revirtió el dato de prueba inmediatamente después de confirmar el bug.

`apps/backend` nunca pasa por PostgREST — se conecta directo a Postgres (`DATABASE_URL`/`DIRECT_URL`) como dueño de las tablas, y en Postgres el dueño de una tabla bypassea RLS salvo que se use `FORCE ROW LEVEL SECURITY` (deliberadamente no usado acá). `ENABLE` sin policies deniega por defecto a cualquier rol que no sea el dueño (`anon`/`authenticated`, los roles que usa PostgREST) — exactamente el comportamiento correcto, dado que toda la autorización real ya vive en NestJS y se confirmó (grep sobre `apps/frontend/src`) que el frontend solo usa `supabase-js` para auth (`.auth.*`), nunca `.from()` sobre estas tablas.

**Verificado después del fix, no solo antes**: el mismo `PATCH` con la `anon key` ahora devuelve `200` con `[]` (0 filas afectadas, RLS filtra todo) y el dato real permanece intacto (confirmado leyendo `GET /courses` del backend real); un smoke test de login real (Admin API → sign in → `GET /auth/me` contra Railway) sigue devolviendo `200` con el perfil sincronizado, confirmando que el backend no se vio afectado; los 51 tests de `apps/backend` siguen en verde.

**Nota**: este bug estaba presente desde que se corrió la migración inicial en Fase 2 — Supabase no habilita RLS por defecto en tablas nuevas, y como `apps/backend` nunca necesitó políticas de RLS (toda su autorización es propia), nunca se pensó en configurarlo hasta que el usuario compartió el reporte de Advisors. Ninguno de los tests/E2E de este proyecto lo hubiera detectado, porque todos pasan por el backend (o por la Admin API con `service_role`, que sí bypassea RLS) — hacía falta específicamente una prueba con la `anon key` contra PostgREST directo, fuera del alcance de la suite existente.

## 2026-07-19 — `migrate dev` queda permanentemente roto para este proyecto; usar `migrate deploy` con SQL escrito a mano

**Decisión**: de acá en más, todo cambio de schema en este repo se escribe a mano como `migration.sql` (siguiendo la convención de carpeta `<timestamp>_<nombre>` de Prisma) y se aplica con `prisma migrate deploy`, nunca con `prisma migrate dev`.

**Motivo**: bug real descubierto tratando de agregar `Level.content` — `prisma migrate dev` falló con `P3006`/`P3018`: `relation "public._prisma_migrations" does not exist` al intentar reproducir el historial de migraciones en la base shadow que usa para diffear el schema. La causa raíz: la migración `enable_row_level_security` (entrada anterior) hace `ALTER TABLE "public"."_prisma_migrations" ...` explícitamente por nombre — pero la base shadow de `migrate dev` es una base temporal que Prisma reconstruye reproduciendo el SQL de cada migración desde cero, y no materializa su propia tabla de bookkeeping `_prisma_migrations` de la misma forma que en una base real; cualquier migración que referencie esa tabla por nombre falla ahí, sin importar qué migraciones vengan después (se intentó agregar una migración correctiva que hace `DISABLE ROW LEVEL SECURITY` sobre esa tabla, aplicada con éxito vía `migrate deploy` contra la base real — confirmado con una lectura real vía PostgREST que `_prisma_migrations` volvió a ser legible y que las 19 tablas de aplicación siguen bloqueadas — pero **no resolvió el problema de `migrate dev`**, porque el fallo ocurre reproduciendo la migración original que rompe, antes de llegar a la corrección).

Dado que dos migraciones ya aplicadas a la base real tocan `_prisma_migrations` y no se pueden reescribir sin invalidar sus checksums, la base shadow de `migrate dev` va a fallar en reproducir el historial completo indefinidamente. La migración `add_level_content` se escribió a mano (`ALTER TABLE "public"."levels" ADD COLUMN "content" JSONB;`) y se aplicó con `migrate deploy`, que no usa base shadow — solo aplica migraciones pendientes contra la base real, comparando contra `_prisma_migrations` de esa misma base. Cualquier cambio de schema futuro debe seguir este mismo patrón manual.

## 2026-07-19 — Contenido curricular gamificado real: pipeline de autoría → seed, no JSON suelto

**Decisión**: el contenido de curso (teoría + aplicación real + mini-quiz por nivel + boss battle por capítulo) se autoría como TypeScript tipado en `packages/database/content/` (`types.ts` define `ChapterContent`/`LevelContent`/`QuizQuestion`/`BossBattleContent`; cada capítulo es un archivo `chapter-NN-slug.ts`), y `prisma/seed.ts` lo consume vía `seedChapterContent()` para escribir filas reales de `Level`/`Assessment` — no queda como JSON estático desconectado de la base.

**Alternativas consideradas**: guardar el contenido como JSON suelto en `apps/frontend/src/data/` (lo que pidió el usuario textualmente), consumido directo por el frontend sin pasar por el backend.

**Motivo**: este proyecto ya tiene un modelo de datos real para currículo (`Course` → `Chapter` → `Level` → `Assessment`/`Lab`) que `apps/backend` sirve vía API y que la Fase 4 (Simulator Engine) ya probó de punta a punta para labs. Poner el contenido en `apps/frontend/src/data/` habría creado una segunda fuente de verdad desconectada del backend, contradiciendo el patrón "cero mocks" del resto del proyecto y obligando a reescribir todo cuando eventualmente se conecte a la base real. Content-as-code en `packages/database/content/` (TypeScript, no JSON crudo, para tener chequeo de tipos real contra `ChapterContent`) vive junto a `seed.ts`, su único consumidor actual, y ya es 100% real: se sembró contra la base de Supabase real y se verificó con una query real (4 `Level` para el Capítulo 1 — 3 sub-temas + 1 boss battle — con sus `Assessment` de tipo QUIZ), no solo se escribió y se asumió que funcionaría.

**Se agregó también** `Level.content` (`Json?`, ver entrada anterior sobre la migración) para guardar las secciones de lectura + aplicación real de cada nivel, y `packages/shared/src/gamification/level-titles.ts` (`levelTitle(playerLevel)`) para el sistema de títulos narrativos que pidió el usuario ("Network Novice" → "Routing Master" en nivel 10 → "Network Grandmaster" como techo en nivel 30) — con tests, separado explícitamente del modelo `Level` de Prisma (currículo) para no confundir "nivel de jugador" con "lección del curso".

**Gap real, explícito, no oculto**: el modelo `Assessment`/`AssessmentAttempt` existe en el schema desde Fase 2 y ahora tiene contenido real cargado, pero **no hay módulo de backend que permita rendir un quiz** (a diferencia de `Lab`/`LabAttempt`, que sí tiene su Simulator Engine completo) ni UI de frontend para leer `Level.content` o tomar un `Assessment`. `GET /courses/:slug` tampoco expone `content`/`assessments` todavía — devuelve solo `id/title/description/order/xpReward/labId` por nivel (confirmado con una request real). El contenido está sembrado, verificado y listo; falta el motor de lectura+quiz para que sea jugable — trabajo de la misma magnitud que el Simulator Engine de Fase 4, fuera de alcance de "generar el contenido".
