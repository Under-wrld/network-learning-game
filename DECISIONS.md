...
**Decisión**: `pnpm-workspace.yaml` agrega `managePackageManagerVersions: false`. Se regeneró `pnpm-lock.yaml` como un único documento YAML válido (se eliminó manualmente el primer documento, de 199 líneas, verificando después con `pnpm install --frozen-lockfile` que el resultado fuera idéntico al que pnpm esperaría — sin cambios de versión reales, solo se quitó el documento espurio).

**Motivo**: bug real encontrado desplegando el frontend a Vercel, no anticipado — el build fallaba con `WARN Ignoring broken lockfile... expected a single document in the stream, but found more`, seguido de una cascada de errores `ERR_INVALID_THIS` al re-resolver dependencias sin lockfile (el pnpm del entorno de build de Vercel, más viejo, ignoraba el lockfile "roto" y intentaba resolver todo desde cero, chocando además con un bug de compatibilidad de ese pnpm más viejo contra el registry). La causa raíz: `pnpm-lock.yaml` tenía **dos documentos YAML concatenados** (dos líneas `---`, línea 1 y línea 199) — el primero era un lockfile separado de solo 197 líneas para `packageManagerDependencies` (`@pnpm/exe`, el binario de pnpm 11.13.1 auto-gestionado como dependencia, activado automáticamente por `devEngines.packageManager.onFail: "download"` en `package.json`), el segundo (a partir de la línea 200) el lockfile real y completo del monorepo. Esto es un feature legítimo y nuevo de pnpm 11 (fijar el binario exacto de pnpm como dependencia reproducible), pero produce un lockfile multi-documento que ni el pnpm del build de Vercel ni (potencialmente) otras herramientas del ecosistema soportan todavía — no hacía falta para este proyecto (Vercel/CI/Docker ya resuelven la versión de pnpm por su cuenta vía el campo `packageManager` + corepack), así que se desactivó en la fuente en vez de trabajar alrededor del síntoma.
